...
# Modified by: David Landy
# Modification notes: now all dots move in a random direction, constrained by expected velocity
...
class DotArea2DA(VisionEgg.Core.Stimulus):
    ...
    def __init__(self, **kw):
        ...
        if self.parameters.mode == "wind":
            self.random_directions_radians = RandomArray.uniform(self.parameters.signal_direction_deg*math.pi/180.0-(math.pi*1/4), \
                                                                 self.parameters.signal_direction_deg*math.pi/180.0+(math.pi*1/4), (num_dots,))

#            self.velocities = RandomArray.uniform(self.parameters.velocity_max-1, self.parameters.velocity_max,(num_dots,))            
            self.velocities = RandomArray.uniform(self.parameters.velocity_max-0, self.parameters.velocity_max,(num_dots,))            
    ...
    def draw(self):
        ...
        if p.on:
            ...
            if self.start_times_sec is not None:
                ...
                #new_random_directions_radians = RandomArray.uniform(0.0,2*math.pi,(len(replace_indices),))
                
	        if self.parameters.mode == "wind":
        	    new_random_directions_radians = RandomArray.uniform(self.parameters.signal_direction_deg*math.pi/180.0-(math.pi*1/4), \
                	                                                 self.parameters.signal_direction_deg*math.pi/180.0+(math.pi*1/4), (len(replace_indices),))

	            new_velocities = RandomArray.uniform(self.parameters.velocity_max-0, self.parameters.velocity_max,(len(replace_indices),))            

	        elif self.parameters.mode == "Gaussian":    
        	    new_random_directions_radians = numpy.random.normal(self.parameters.signal_direction_deg/180.0*math.pi, self.parameters.signal_fraction,(len(replace_indices),))
	            new_velocities = RandomArray.uniform(self.parameters.velocity_min, self.parameters.velocity_max,(len(replace_indices),))

                Numeric.put( self.random_directions_radians, replace_indices, new_random_directions_radians )
                Numeric.put( self.velocities, replace_indices, new_velocities )
            ...
            random_x_increment_normalized =  Numeric.cos(self.random_directions_radians[:]) * self.velocities / p.size[0] * time_delta_sec
            random_y_increment_normalized = -Numeric.sin(self.random_directions_radians[:]) * self.velocities / p.size[1] * time_delta_sec
            self.x_positions[:] += random_x_increment_normalized
            self.y_positions[:] += random_y_increment_normalized

            if(p.mode == "wind"):
                self.x_positions[:] +=  Numeric.cos(p.signal_direction_deg/180.0*math.pi) * p.signal_fraction*self.velocities / p.size[0] * time_delta_sec
                self.y_positions[:] += -Numeric.sin(p.signal_direction_deg/180.0*math.pi) * p.signal_fraction*self.velocities / p.size[0] * time_delta_sec
